...
def search_contact(f):
    ...
    if not adr_book:
        ...
        par_choice = input(
            'Укажите, что хотите изменить: 1 - Фамилию, 2 - Имя, 3 - Номер телефона -- ')
...
def clear_phone_book(f):
    ...
    if not adr_book:
        ...
        act_choice = input(
            'Выберите действие: 1 - очистить книжку, 2 - удалить контакт ')
        if act_choice == '1':
            adr_book.clear()
        elif act_choice == '2':
            for i in range(len(adr_book)):
                print(f'{i+1} - {adr_book[i]}')
            idx = int(
                input('Укажите номер контакта, который хотите удалить: ')) - 1
            del adr_book[idx]
